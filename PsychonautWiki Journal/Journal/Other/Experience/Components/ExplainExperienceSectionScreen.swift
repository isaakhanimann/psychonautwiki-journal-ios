...
struct TimelineExplanationTexts: View {
    var body: some View {
        Group {
            ...
            Text("The timelines from ingestions with the same substance and administration route where substances have onset, comeup, peak and offset defined are combined into a cumulative timeline by simply putting the individual lines on top of each other. This is not done for substances/routes where either the onset, comeup, peak or offset duration is missing or if the \"draw redoses individually\" toggle is enabled.")
            ...
            Text("The logic of how high the timeline is, is as follows: all ingestions are converted to a normalized dose which is 1 for the minimum of the common dose range. 1.5 would mean 1.5 times the common min dose. Unknown doses are estimated as 1 = common min dose. Doses where there is no PW dose range as e.g. for custom substances or other routes of administration take the average of the doses in the experience as the common min dose. This allows to not only compare doses of ingestions of the same substance and route of administration but also compare ingestions of different substances. The ingestion or cumulative timeline with the highest normalized dose in the experience always takes the full height. The other ingestions are sized relative to that one.")
